...
import { AfficherVol } from "./actionsThunk";
import { Link } from "react-router-dom";
import './InfosBVoyage.css';
import { Fade } from "react-reveal";

export default function VolOrg() {
    ...
    const handleSearch = () => {
        const filteredVols = vols.filter(vol => {
            const destinationMatches = destination === '' || vol.nomVille.toLowerCase() === destination.toLowerCase();
            const dateDepartMatches = dateDepart === '' || vol.date_depart === dateDepart;
            const dateArriveeMatches = dateArrivee === '' || vol.date_arrivee === dateArrivee;
            return destinationMatches && dateDepartMatches && dateArriveeMatches;
        });
        setFilteredVols(filteredVols);
    };
    return (
        <>
            <div className="banner_section layout_padding">
            <div className="banner_section layout_padding" id="content"  style={{backgroundImage: "url('images/vol-nuit.jpg')",backgroundRepeat:'no-repeat' , backgroundSize: 'cover'}}>
                        ...
                        <div className="carousel-inner">
                            <div className="carousel-item active">
                                <div className="row">
                                    ...
                                    <h1 className="banner_taital">Recherchez votre vol préféré</h1>
                        ...
                        <div className="row">
                            <div className="col-sm-12">
                                <div className="Properties_taital_main">
                                    <h1 className="Properties_taital">Vols Disponibles</h1>
                                    <hr className="border_main" />
                                </div>
                            </div>
                        </div>
                        <div className="row">
                            {filteredVols.map((i, index) => (
                                <Fade top key={index}>
                                    <div className="col-lg-4 col-md-6">
                                        <div className="Properties_section_2">
                                            <Link to={`/vol-sel/${i.idVol}`}>
